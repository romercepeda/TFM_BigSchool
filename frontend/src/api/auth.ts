...
import type { LoginResponse, LoginUserOut, UserResponse } from './types.js';

export interface LoginBody { email: string; password: string; }
export interface RegisterBody { email: string; password: string; display_name?: string; }
...
export const changePassword = (body: ChangePasswordBody) =>
  post<LoginUserOut>('/auth/change-password', body);
