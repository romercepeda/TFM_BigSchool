...
Se han creado dos cuentas de demostración en el entorno de desarrollo local, una por cada rol del sistema:

| Rol | Email |
|---|---|
| **Administrador** | `admin.demo@portfolioia.local` |
| **Inversor** | `investor.demo@portfolioia.local` |

Las contraseñas de estas cuentas no se publican en este README; se comparten por un canal privado.

- La cuenta **Administrador** tiene acceso completo, incluida la sección "Administración" (gestión de usuarios y roles, panel de fallos de proveedores de datos).
- La cuenta **Inversor** representa al usuario típico: solo puede gestionar sus propias carteras, activos, niveles de precio y análisis.

> Además de esta cuenta de demo, el sistema crea automáticamente **su propio administrador inicial** (`admin@portfolioia.local` por defecto, configurable en `config.yaml` → `security.default_admin_email`) la primera vez que arranca el backend sin ningún administrador existente. La contraseña generada se muestra **una sola vez** en el log de arranque; revísalo si necesitas esa cuenta en lugar de la de demo.
